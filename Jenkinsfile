    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Authorize DevHub') {
                rc = command "sfdx help" //"${toolbelt}/sfdx force:auth:jwt:grant --instanceurl ${SF_INSTANCE_URL} --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile ${server_key_file} --setdefaultdevhubusername --setalias HubOrg"
                if (rc != 0) {
	    ...
	    // -------------------------------------------------------------------------
          ...
          //      rc = command "${toolbelt}/sfdx force:source:deploy --targetusername ciorg"
          //      if (rc != 0) {
          //          error 'Salesforce push to Sandbox failed.'
          //      }
          //  }
